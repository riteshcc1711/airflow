...
class CSV_Validation:
	...
	def validate_csv(df,file_data,csv_file_name):
		try:
			...
			if (set(df.columns.values) == set(column_names)) and (len(df.columns.values) == no_of_columns) and (file_name == csv_file_name) :
